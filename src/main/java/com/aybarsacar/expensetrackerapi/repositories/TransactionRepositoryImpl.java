...
public class TransactionRepositoryImpl implements TransactionRepository {
  ...
  public static final String SQL_CREATE = "INSERT INTO ET_TRANSACTIONS " +
      "(TRANSACTION_ID, CATEGORY_ID, USER_ID, AMOUNT, NOTE, TRANSACTION_DATE) " +
      "VALUES(NEXTVAL('ET_TRANSACTIONS_SEQ'), ?, ?, ?, ?, ?)";

  public static final String SQL_FIND_BY_ID = "SELECT TRANSACTION_ID, CATEGORY_ID, USER_ID, " +
      "AMOUNT, NOTE, TRANSACTION_DATE FROM ET_TRANSACTIONS " +
      "WHERE USER_ID = ? AND CATEGORY_ID = ? AND TRANSACTION_ID = ?";

  public static final String SQL_FIND_ALL = "SELECT TRANSACTION_ID, CATEGORY_ID, USER_ID, " +
      "AMOUNT, NOTE, TRANSACTION_DATE FROM ET_TRANSACTIONS " +
      "WHERE USER_ID = ? AND CATEGORY_ID = ?";

  public static final String SQL_UPDATE = "UPDATE ET_TRANSACTIONS " +
...
//    if the count is 0 no rows been deleted
    if (count == 0) {
      throw new EtResourceNotFoundException("Transaction not found");
    }
